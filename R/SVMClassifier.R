...
SVMClassifier <- function(X,
                          ...
                          cells.use = NULL,
                          probability = FALSE,
                          kernel = c("laplacedot", "rbfdot", "vanilladot"),
                          type = c("spoc-svc", "C-svc", "C-bsvc"),
                          C = 1,
                          cross = 0,
                          do.scale = F,
  ...
  # training SVM
  classifier <- ksvm(debatch.catalogue,
                     catalogue.celltype,
                     kernel = kernel,
                     type = type,
  ...
  # predict
  if (probability){
    y <- predict(classifier, debatch.X, type="probabilities")
  }else{
