...
#' @examples
#'
RandomForestClassifier <- function(X,
                                   catalogue.profile,
                                   catalogue.celltype,
                                   ...
                                   BPPARAM = SerialParam())
{
  ...
  train.data <- data.frame(debatch.catalogue)
  train.data$y <- catalogue.celltype
  classifier <- ranger(y~., data = train.data, probability = probability)

  # predict
  y <- predict(classifier, data.frame(debatch.X))
  y <- y$predictions
